...
export default function IpadCategoryPage() {
  const lines = getIpadLines();
    ...
    <div className="bg-white">
      <div className="mx-auto max-w-[1920px] px-4 py-6 md:px-6 md:py-8">
        ...
        >
          ...
          <span className="font-medium text-[#221f1f]">iPad</span>
        </nav>

        <h1 className="loom-promo-section-title mb-8">iPad</h1>

        <ul className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-3">
          {lines.map((line) => (
            <li key={line.slug}>
              <Link
                href={`/category/ipad/${line.firstModelSlug}`}
                className={cn(
